...
litmus_dir = os.path.join(test_dir, 'delay_litmus')
...
def statistic_state():
    log_files = []
    for dirpath, _, filenames in os.walk(userfence_dir):
    ...
    for log_file in log_files:
        ...
        state_dict = {r.name for r in state_dict}[litmus_name_without_suffix]
        for state in state_dict.states:
            state_num = state.num
            if str(state) not in log[litmus_name]:
                log[litmus_name][str(state)] = []
            log[litmus_name][str(state)].append((litmus_name, state_num))
    for litmus_name_without_suffix in log:
        for state in log[litmus_name_without_suffix]:
            txt_dir = os.path.join(test_dir, f'delay_st/{litmus_name_without_suffix}_{state}.txt')
            with open(txt_dir, 'w') as f:
                sorted_data = sorted(log[litmus_name_without_suffix][state], key=lambda x: int(x[1]))
                for litmus_name, state_num in sorted_data:
                    f.write(litmus_name+' : '+str(state_num)+'\n')
                    f.write('\n')


if __name__ == '__main__':
    files = [f for f in os.listdir(litmus_dir) if os.path.isfile(os.path.join(litmus_dir, f))]
    chip_dir = '/home/sipeed/delay'
    # files = ['SB.litmus']
