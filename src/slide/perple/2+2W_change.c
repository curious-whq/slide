...
static void *P0(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      barrier_wait(_th_id,_i,&barrier[_i]);asm __volatile__ (
      "#START _litmus_P0\n"
      "#_litmus_P0_0\n\t"
      "#_litmus_P0_1\n\t"
      "sw %[x5],0(%[x6])\n"
...
for (int perple_a = 0; perple_a <= _b->size_of_test - 1 ; perple_a++){
  for (int perple_b = 0; perple_b <= _b->size_of_test - 1 ; perple_b++){
    if (ctx.out_0_x9[perple_a] % 2 == 0 && ctx.out_0_x9[perple_a]<=2*(perple_b)+2 && ctx.out_1_x9[perple_b] % 2 == 0 && ctx.out_1_x9[perple_b]<=2*(perple_a)+2){
      counter_exhaust += 1;
    }
  }
}
...
static void postlude(FILE *out,cmd_t *cmd,hist_t *hist,count_t p_true,count_t p_false,tsc_t total) {
  ...
  fprintf(out,"Condition %s is %svalidated\n","exists (0:x9=2 /\\ 1:x9=2)",cond ? "" : "NOT ");
  fprintf(out,"Hash=36f371fd6ed20823f66353b6a7a64071\n");
  fprintf(out,"Generator=diyone7 (version 7.56)\n");
  fprintf(out,"Com=Co Co\n");
  fprintf(out,"Orig=PodWW Coe PodWW Coe\n");
