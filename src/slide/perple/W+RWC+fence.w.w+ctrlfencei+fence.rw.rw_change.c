...
static void *P2(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "#END _litmus_P2\n\t"
      :[x7] "=&r" (out_2_x7[_i])
      :[x5] "r" (x5_val),[x6] "r" (&_a->z[_i]),[x8] "r" (&_a->x[_i])
      :"cc","memory"
      );x5_val += 1;
...
for (int perple_a = 0; perple_a <= _b->size_of_test - 1 ; perple_a++){
  for (int perple_b = 0; perple_b <= _b->size_of_test - 1 ; perple_b++){
    for (int perple_c = 0; perple_c <= _b->size_of_test - 1 ; perple_c++){
      if (ctx.out_1_x5[perple_b]>=1*(perple_a)+1 && ctx.out_1_x7[perple_b]<=1*(perple_c-1)+1 && ctx.out_2_x7[perple_c]<=1*(perple_a-1)+1){
        counter_exhaust += 1;
      }
    }
  }
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/whq/Desktop/code_list/perple_test/all_allow_litmus_C910_naive_perple/W+RWC+fence.w.w+ctrlfencei+fence.rw.rw.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV W+RWC+fence.w.w+ctrlfencei+fence.rw.rw");
  fprintf(out,"%s\n","\"Fence.w.wdWW Rfe DpCtrlFenceIdR Fre Fence.rw.rwdWR Fre\"");
  fprintf(out,"%s\n","{0:x5=1; 0:x6=x; 0:x7=y; 1:x6=y; 1:x8=z; 2:x5=1; 2:x6=z; 2:x8=x;}");
  fprintf(out,"%s\n"," P0          | P1             | P2          ;");
