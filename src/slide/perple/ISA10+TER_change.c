...
static void *P1(void *_vb) {
  ...
  int *out_1_x12 = _a->out_1_x12;
  int *out_1_x14 = _a->out_1_x14;
  for (int _j = _stride ; _j > 0 ; _j--) {
    int trashed_x11 = 1;
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      int trashed_x7;
      int trashed_x10;      int trashed_x13;
      int trashed_x16;
      asm __volatile__ (
      "#START _litmus_P1\n"
      "#_litmus_P1_0\n\t"
      ...
      "lw %[x14],0(%[x16])\n"
      "#END _litmus_P1\n\t"
      :[x14] "=&r" (out_1_x14[_i]),[x12] "=&r" (out_1_x12[_i]),[x8] "=&r" (out_1_x8[_i]),[x5] "=&r" (out_1_x5[_i]),[x16] "=&r" (trashed_x16),[x13] "=&r" (trashed_x13),,[x10] "=&r" (trashed_x10),[x7] "=&r" (trashed_x7)
      :[x6] "r" (&_a->y[_i]),[x9] "r" (&_a->z[_i]),[x15] "r" (&_a->x[_i]), [x11] "r" (trashed_x11)
      :"cc","memory"
      );trashed_x11 += 1;
...
for (int perple_a = 0; perple_a <= _b->size_of_test - 1 ; perple_a++){
  for (int perple_b = 0; perple_b <= _b->size_of_test - 1 ; perple_b++){
    if (ctx.out_1_x5[perple_b]>=1*(perple_a)+1 && ctx.out_1_x14[perple_b]<=1*(perple_a-1)+1){
      counter_exhaust += 1;
    }
  }
}
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/whq/Desktop/code_list/slide/src/slide/../../tests/input/litmus/non-mixed-size/HAND/ISA10+TER.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV ISA10+TER");
