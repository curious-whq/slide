...
static void *P0(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "lw %[x2],0(%[x3])\n"
      "#END _litmus_P0\n\t"
      :[x9] "=&r" (out_0_x9[_i]),[x2] "=&r" (out_0_x2[_i])
      :[x3] "r" (&_a->z[_i]),[x6] "r" (&_a->x[_i]),[x7] "r" (&_a->y[_i]), [x8] "r" (trashed_x8), [x5] "r" (trashed_x5)
      :"cc","memory"
      );trashed_x5 += 1;
...
static void *P1(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "#END _litmus_P1\n\t"
      :[x7] "=&r" (out_1_x7[_i])
      :[x3] "r" (&_a->z[_i]),[x6] "r" (&_a->y[_i]), [x1] "r" (trashed_x1)
      :"cc","memory"
      );trashed_x1 += 1;
...
for (int perple_a = 0; perple_a <= _b->size_of_test - 1 ; perple_a++){
  for (int perple_b = 0; perple_b <= _b->size_of_test - 1 ; perple_b++){
    if (ctx.out_0_x2[perple_a]>=1*(perple_b)+1 && ctx.out_1_x7[perple_b]<=1*(perple_a-1)+1){
      counter_exhaust += 1;
    }
  }
}
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/whq/Desktop/code_list/slide/src/slide/../../tests/input/litmus/non-mixed-size/HAND/C-Will02+HEAD.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV C-Will02+HEAD");
