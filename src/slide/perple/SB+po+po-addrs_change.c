...
static void *P1(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "lw %[x10],0(%[x11])\n"
      "#END _litmus_P1\n\t"
      :[x10] "=&r" (out_1_x10[_i]),[x11] "=&r" (trashed_x11),[x9] "=&r" (trashed_x9),[x7] "=&r" (trashed_x7)
      :[x5] "r" (x5_val),[x6] "r" (&_a->y[_i]),[x8] "r" (&_a->x[_i])
      :"cc","memory"
      );x5_val += 1;
...
for (int perple_a = 0; perple_a <= _b->size_of_test - 1 ; perple_a++){
  for (int perple_b = 0; perple_b <= _b->size_of_test - 1 ; perple_b++){
    if (ctx.out_0_x7[perple_a]<=1*(perple_b-1)+1 && ctx.out_1_x10[perple_b]<=1*(perple_a-1)+1){
      counter_exhaust += 1;
    }
  }
}
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/whq/Desktop/code_list/slide/src/slide/../../tests/input/litmus/non-mixed-size/RELAX/PodWR/SB+po+po-addrs.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
