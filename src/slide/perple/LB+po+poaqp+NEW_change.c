...
static void *P0(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "#END _litmus_P0\n\t"
      :[x5] "=&r" (out_0_x5[_i])
      :[x6] "r" (&_a->x[_j]),[x7] "r" (x7_val),[x8] "r" (&_a->y[_j])
      :"cc","memory"
      );x7_val += 1;
...
static void *P1(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "#END _litmus_P1\n\t"
      :[x5] "=&r" (out_1_x5[_i])
      :[x6] "r" (&_a->y[_j]),[x7] "r" (x7_val),[x8] "r" (&_a->x[_j])
      :"cc","memory"
      );x7_val += 1;
...
#define REVERSE_CTX(arr, n) do {          int _i = 0, _j = (n) - 1;             while (_i < _j) {                         __typeof__(arr[0]) tmp = arr[_i];         arr[_i] = arr[_j];                    arr[_j] = tmp;                        _i++;                                 _j--;                             }                                 } while (0)
...
for (int perple_a = 0; perple_a <= _b->size_of_test - 1 ; perple_a++){
  for (int perple_b = 0; perple_b <= _b->size_of_test - 1 ; perple_b++){
    if (ctx.out_1_x5[perple_b]>=1*(perple_a)+1 && ctx.out_0_x5[perple_a]>=1*(perple_b)+1){
      counter_exhaust += 1;
    }
  }
}
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/whq/Desktop/code_list/slide/src/slide/../../tests/input/litmus/non-mixed-size/AMO_X0_2_THREAD/LB+po+poaqp+NEW.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
