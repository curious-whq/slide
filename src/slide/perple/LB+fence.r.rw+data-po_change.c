...
static void *P0(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "#END _litmus_P0\n\t"
      :[x5] "=&r" (out_0_x5[_i])
      :[x6] "r" (&_a->x[_i]),[x8] "r" (&_a->y[_i]), [x7] "r" (trashed_x7)
      :"cc","memory"
      );trashed_x7 += 1;
...
static void *P1(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      int trashed_x7;asm __volatile__ (
      "#START _litmus_P1\n"
      "#_litmus_P1_0\n\t"
      "lw %[x5],0(%[x6])\n"
      "#_litmus_P1_1\n\t"
      ...
      "sw %[x9],0(%[x10])\n"
      "#END _litmus_P1\n\t"
      :[x5] "=&r" (out_1_x5[_i]),[x7] "=&r" (trashed_x7)
      :[x6] "r" (&_a->y[_i]),[x8] "r" (&_a->z[_i]),[x10] "r" (&_a->x[_i]), [x9] "r" (trashed_x9)
      :"cc","memory"
      );trashed_x9 += 1;
...
reverse_ctx(ctx.out_0_x5, _b->size_of_test);
reverse_ctx(ctx.out_1_x5, _b->size_of_test);
...
for (int perple_a = 0; perple_a <= _b->size_of_test - 1 ; perple_a++){
  for (int perple_b = 0; perple_b <= _b->size_of_test - 1 ; perple_b++){
    if (ctx.out_0_x5[perple_a]>=1*(perple_b)+1 && ctx.out_1_x5[perple_b]>=1*(perple_a)+1){
      counter_exhaust += 1;
    }
  }
}
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/whq/Desktop/code_list/slide/src/slide/../../tests/input/litmus/non-mixed-size/HAND/LB+fence.r.rw+data-po.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
