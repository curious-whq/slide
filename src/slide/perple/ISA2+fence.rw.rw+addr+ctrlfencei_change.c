...
static void *P0(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "#END _litmus_P0\n\t"
      :
      :[x5] "r" (x5_val),[x6] "r" (&_a->x[_i]),[x7] "r" (&_a->y[_i])
      :"cc","memory"
      );x5_val += 1;
...
static void *P1(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "sw %[x8],0(%[x10])\n"
      "#END _litmus_P1\n\t"
      :[x5] "=&r" (out_1_x5[_i]),[x10] "=&r" (trashed_x10),[x7] "=&r" (trashed_x7)
      :[x6] "r" (&_a->y[_i]),[x8] "r" (x8_val),[x9] "r" (&_a->z[_i])
      :"cc","memory"
      );x8_val += 1;
...
static void *P2(void *_vb) {
  ...
  int _size_of_test = _a->_p->size_of_test;
  int _stride = _a->_p->stride;
  int *out_2_x5 = _a->out_2_x5;
  int *out_2_x7 = _a->out_2_x7;
  for (int _j = _stride ; _j > 0 ; _j--) {
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      barrier_wait(_th_id,_i,&barrier[_i]);
      asm __volatile__ (
      ...
      "lw %[x7],0(%[x8])\n"
      "#END _litmus_P2\n\t"
      :[x7] "=&r" (out_2_x7[_i]),[x5] "=&r" (out_2_x5[_i])
...
for (int perple_a = 0; perple_a <= _b->size_of_test - 1 ; perple_a++){
  for (int perple_b = 0; perple_b <= _b->size_of_test - 1 ; perple_b++){
    for (int perple_c = 0; perple_c <= _b->size_of_test - 1 ; perple_c++){
      if (ctx.out_1_x5[perple_b]>=1*(perple_a)+1 && ctx.out_2_x5[perple_c]>=1*(perple_b)+1 && ctx.out_2_x7[perple_c]<=1*(perple_a-1)+1){
        counter_exhaust += 1;
      }
    }
  }
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/whq/Desktop/code_list/slide/src/slide/../../tests/input/litmus/non-mixed-size/SAFE/ISA2+fence.rw.rw+addr+ctrlfencei.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV ISA2+fence.rw.rw+addr+ctrlfencei");
  fprintf(out,"%s\n","\"Fence.rw.rwdWW Rfe DpAddrdW Rfe DpCtrlFenceIdR Fre\"");
  fprintf(out,"%s\n","{0:x5=1; 0:x6=x; 0:x7=y; 1:x6=y; 1:x8=1; 1:x9=z; 2:x6=z; 2:x8=x;}");
  fprintf(out,"%s\n"," P0          | P1            | P2             ;");
