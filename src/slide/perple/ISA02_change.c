...
static void *P0(void *_vb) {
  ...
  int *out_0_x10 = _a->out_0_x10;
  int *out_0_x11 = _a->out_0_x11;
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "lw %[x11],0(%[x9])\n"
      "#END _litmus_P0\n\t"
      :[x11] "=&r" (out_0_x11[_i]),[x10] "=&r" (out_0_x10[_i])
      :[x8] "r" (&_a->x[_i]),[x9] "r" (&_a->y[_i]), [x6] "r" (trashed_x6)
      :"cc","memory"
      );trashed_x6 += 1;
...
static void *P1(void *_vb) {
  ...
  for (int _j = _stride ; _j > 0 ; _j--) {
    ...
    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      ...
      "lw %[x13],0(%[x8])\n"
      "#END _litmus_P1\n\t"
      :[x13] "=&r" (out_1_x13[_i]),[x12] "=&r" (out_1_x12[_i])
      :[x8] "r" (&_a->x[_i]),[x9] "r" (&_a->y[_i]), [x6] "r" (trashed_x6)
      :"cc","memory"
      );trashed_x6 += 1;
...
void reverse_ctx(int *arr, int n) {
    int i = 0, j = n - 1;
    while (i < j) {
        int tmp = arr[i];
        arr[i] = arr[j];
...
reverse_ctx(ctx.out_0_x11, _b->size_of_test);
reverse_ctx(ctx.out_1_x13, _b->size_of_test);
...
for (int perple_a = 0; perple_a <= _b->size_of_test - 1 ; perple_a++){
  for (int perple_b = 0; perple_b <= _b->size_of_test - 1 ; perple_b++){
    if (ctx.out_0_x11[perple_a]<=1*(perple_b-1)+1 && ctx.out_1_x13[perple_b]<=1*(perple_a-1)+1){
      counter_exhaust += 1;
    }
  }
}
...
static void prelude(FILE *out) {
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","% Results for /home/whq/Desktop/code_list/slide/src/slide/../../tests/input/litmus/non-mixed-size/HAND/ISA02.litmus %");
  fprintf(out,"%s\n","%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
  fprintf(out,"%s\n","RISCV ISA02");
