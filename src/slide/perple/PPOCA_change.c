...
static void *P1(void *_vb) {
  ...
  int *out_1_x11 = _a->out_1_x11;
  for (int _j = _stride ; _j > 0 ; _j--) {
    int trashed_x7 = 1;

    for (int _i = _size_of_test-_j ; _i >= 0 ; _i -= _stride) {
      barrier_wait(_th_id,_i,&barrier[_i]);      int trashed_x10;
      int trashed_x13;
      asm __volatile__ (
      "#START _litmus_P1\n"
      "#_litmus_P1_0\n\t"
      ...
      "0:\n"
      "#END _litmus_P1\n\t"
      :[x11] "=&r" (out_1_x11[_i]),[x9] "=&r" (out_1_x9[_i]),[x5] "=&r" (out_1_x5[_i]),[x13] "=&r" (trashed_x13),[x10] "=&r" (trashed_x10)
      :[x6] "r" (&_a->y[_i]),[x8] "r" (&_a->z[_i]),"[x9]" (0),"[x11]" (0),[x12] "r" (&_a->x[_i]), [x7] "r" (trashed_x7)
      :"cc","memory"
      );trashed_x7 += 1;
...
for (int perple_a = 0; perple_a <= _b->size_of_test - 1 ; perple_a++){
  for (int perple_b = 0; perple_b <= _b->size_of_test - 1 ; perple_b++){
    if (ctx.out_1_x5[perple_b]>=1*(perple_a)+1 && ctx.out_1_x11[perple_b]<=1*(perple_a-1)+1){
      counter_exhaust += 1;
    }
  }
}
