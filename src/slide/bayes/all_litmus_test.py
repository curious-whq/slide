...
def stat_result():
    ...
    for dir in dir_list:
        ...
        litmus_max_dict[dir_name] = 0
        time_dict[dir_name] = 0
        if os.path.exists(os.path.join(dir, "result.log")):
            with open(os.path.join(dir, "result.log"), "r") as fl:
                results = fl.readlines()
                for result in results:
                    if len(result.split(':'))==2 and "litmus result" not in result and "not compile litmus" not in result:
                        litmus_name = result.split(":")[0]
                        num = int(result.split(":")[1].split(',')[0].strip())
                        time = float(result.split(":")[1].split(',')[1].strip())
                        if litmus_name not in litmus_dict:
                            litmus_dict[litmus_name] = []
                        litmus_dict[litmus_name].append((dir_name, num, time))
    for litmus_name in litmus_dict:
        litmus_dict[litmus_name].sort(key=lambda x: x[2])
        if litmus_dict[litmus_name][-1][1] != 0:
            litmus_max_dict[litmus_dict[litmus_name][-1][0]]+=1
            if litmus_dict[litmus_name][-1][2] != 0:
                time = litmus_dict[litmus_name][-1][1]/litmus_dict[litmus_name][-1][2]
                num = litmus_dict[litmus_name][-1][1]
                if num >= 30:
                    power = int(math.log10(num // 3))  # 根据数量级自动确定除数
                    divisor = 10 ** power
                    time = time / divisor
                time_dict["max"]+= 3/litmus_dict[litmus_name][-1][2]
                time_dict["ori"]+= time
    with open(log_path, "w") as fl:
        for litmus_name in litmus_dict:
            fl.write(f"{litmus_name} result: \n")
            for dir_name, litmus_num, times in litmus_dict[litmus_name]:
                fl.write(f"{dir_name}: {litmus_num}, {times}\n")
                if times != 0:
                    time_dict[dir_name] += 3/times
        fl.write("\n")
        fl.write("litmus stats: \n")
        for dir_name in litmus_max_dict:
            fl.write(f"{dir_name}: {litmus_max_dict[dir_name]}\n")
        for dir_name in time_dict:
            fl.write(f"{dir_name}: {time_dict[dir_name]}\n")

out_path ="/home/whq/Desktop/code_list/perple_test/all_litmus_perple"
in_path = "/home/whq/Desktop/code_list/perple_test/all_allow_litmus_C910_naive_perple"
log_path = "/home/whq/Desktop/code_list/perple_test/log_C910/log.txt"
chip_dir = "/home/sipeed/all"
dir_list = [
    "/home/whq/Desktop/code_list/perple_test/all_allow_litmus",
    "/home/whq/Desktop/code_list/perple_test/all_allow_litmus_C910_fence_none",
    "/home/whq/Desktop/code_list/perple_test/all_allow_litmus_C910_fence_timebase",
    "/home/whq/Desktop/code_list/perple_test/all_allow_litmus_C910_contiguous_true",
    "/home/whq/Desktop/code_list/perple_test/all_allow_litmus_C910_alloc_before",
    "/home/whq/Desktop/code_list/perple_test/all_allow_litmus_C910_mem_indirect",
    "/home/whq/Desktop/code_list/perple_test/all_allow_litmus_C910_noalign_all",
]
...
if __name__ == "__main__":
    ...
    #
    # # filter_allowed_litmus(litmus_files)
    ...
    for litmus_file in litmus_files:
        ...
        # cmd += f" -o {chip_dir}"
        # print(cmd)
        cmd = f"mkdir -p {out_path}/{litmus_name};eval $(opam env); litmus7 -carch RISCV -limit true -mem direct -affinity incr1 -barrier none -stride 1 -size_of_test 100 -number_of_run 10 -driver C -ccopts -O2 -gcc riscv64-unknown-linux-gnu-gcc -linkopt -static -smtmode seq -smt 2  -avail 4 "
    ...
    for root, dirs, files in os.walk(out_path):
        for d in dirs:
            ...
            for litmus_path in all_paths:
                if litmus_path.split("/")[-1][:-7] == litmus_name:
                    print(litmus_path)
                    if litmus_name in ["ISA-MP-DEP-SUCCESS-SUCCESS", "ISA03+SB02", "OpVsHerd01", "LB+addr+addrpx-poxp+VAR2", "ISA03"]:
                        num += 1
                        continue
