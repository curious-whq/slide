...
class RandomGridRunner(LitmusRunner):
    def __init__(
        ...
        json_file_path = "/home/whq/Desktop/code_list/perple_test/bayes_stat/log_record_bayes.log.cache_sum_70_filter.jsonl"  # 替换为你的实际文件路径
        self.todo_queue = []

        self.logger.info(f"Loading litmus tasks from {json_file_path}...")
        ...
        try:
            with open(json_file_path, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    param_vector = data.get("param")

                    # 将 (litmus名字, 向量参数) 元组加入队列
                    # 注意：如果你的下游逻辑需要特定的 litmus 对象，
                    # 请确保 litmus_name 能对应到相应的对象。
                    self.todo_queue.append((litmus_name, param_vector))

            self.logger.info(f"Successfully loaded {len(self.todo_queue)} tasks.")
